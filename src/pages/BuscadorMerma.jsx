...
export default function BuscadorMerma({ user, rol, onBack, onNavegar, rolReal, setRolSimulado, isInline = false }) {
  ...
  if (isInline) {
    return (
      <div ref={mainRef} className="flex flex-col flex-1 min-w-0 relative animate-in fade-in slide-in-from-bottom-4 duration-500">
        {/* ── HEADER ── */}
        <header className={`pb-6 flex flex-col gap-5 border-b ${t.border} flex-shrink-0`}>
          {/* Título + botones */}
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
            <div className="flex items-start gap-4">
              <div className={`mt-1 w-10 h-10 md:w-11 md:h-11 flex items-center justify-center rounded-full ${t.bgCard} border ${t.border} ${t.textSecondary} shadow-sm transition-all`}>
                <span className="material-symbols-outlined text-blue-500/80" style={{ fontSize: 24, fontVariationSettings: "'FILL' 1" }}>
                  inventory_2
                </span>
              </div>
              <div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 mt-4">
            <div className="relative flex-1 group">
              ...
              />
              {search && (
                <button type="button" onClick={() => setSearch("")} className={`absolute right-3 top-1/2 -translate-y-1/2 ${t.textSecondary} hover:text-red-400 transition-colors`}>
        ...
        {/* ── GRID ── */}
        <section className="flex-1 pt-6 pb-24 md:pb-8">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 gap-3">
              ...
              {(search || catActiva) && (
                <button type="button" onClick={() => { setSearch(""); setCatActiva(""); }} className="text-blue-400 text-sm hover:underline">
                  Limpiar filtros
                </button>
              )}
            </div>
            ...
            <>
              <p className={`${t.textSecondary} text-sm mb-5`}>
                ...
                {sinCategoria > 0 && (
                  <button type="button" onClick={() => setCatActiva("__sin__")}
              ...
              {totalPaginas > 1 && (
                <div className="flex items-center justify-between pt-6 mt-4 border-t" style={{ borderColor: "inherit" }}>
                  ...
                  <div className="flex items-center gap-2 mx-auto sm:mx-0">
                    ...
                    {getBotones().map((n) => (
                      <button type="button" key={n} onClick={() => cambiarPagina(n)}
