...
export default function SizzlingModule({ rol }) {
  const { t } = useTheme();
  const { user } = useAuth();
  ...
  const [itemsMaterias, setItemsMaterias] = useState(['Insumo S1', 'Insumo S2']);
  const [itemsProduccion, setItemsProduccion] = useState(['Producto S1', 'Producto S2']);
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(doc(db, coll, 'sizzling'), (snap) => {
  ...
  const handleSaveChecklist = async () => {
    const coll = activeTab === 'materias' ? 'trazabilidad_materias' : 'trazabilidad_produccion';
    try {
      await setDoc(doc(db, coll, 'sizzling'), { ...data, updatedAt: new Date(), updatedBy: user?.email });
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div><h1 className={`${t.text} text-2xl font-black tracking-tight leading-none mb-1 uppercase`}>{activeTab === 'materias' ? 'Materias Primas' : 'Producción'}</h1><p className={`${t.textSecondary} text-[10px] uppercase font-black tracking-[0.2em] opacity-60`}>Sizzling</p></div>
        </div>
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {activeTab === 'pcc' && <CoccionEnfriadoModule rol={rol} cuarto="Sizzling" />}
      {activeTab === 'sanitizacion' && <SanitizacionModule rol={rol} cuarto="Sizzling" />}
    </div>
  );
}
