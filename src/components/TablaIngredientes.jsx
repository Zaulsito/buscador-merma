...
export default function TablaIngredientes({ titulo, lista, campo, onUpdate, onAgregar, onAgregarVarios, onEliminar, placeholder = "Ingrediente", conUnidad = false }) {
  const { t } = useTheme();
  ...
  const esEncabezado = (item) => !item.unidad || item.unidad === "";
  ...
  return (
    <div className="mb-6">
      ...
      {lista.map((item, i) => (
        esEncabezado(item) ? (
          <div key={i} className={`grid ${cols} gap-2 mb-2 items-center`}>
            ...
            {(item.unidad === "UN/KG" || item.unidad === "L/KG") ? (
              <input
                ...
                onChange={(e) => {
                  ...
                  const p1 = formatearEntero(partes[0] || "");
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter") {
                    e.preventDefault();
                    const siguiente = document.querySelectorAll(`[data-bruta="${campo}"]`)[i + 1];
                    if (siguiente) siguiente.focus();
                  }
                }}
                data-bruta={campo}
              ...
              <input
                value={item.cantidadBruta}
                onChange={(e) => {
                  const nuevo = formatearEntero(e.target.value);
            ...
            {(item.unidad === "UN/KG" || item.unidad === "L/KG") ? (
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter") {
                    e.preventDefault();
                    const siguiente = document.querySelectorAll(`[data-neta="${campo}"]`)[i + 1];
                    if (siguiente) siguiente.focus();
                  }
                }}
                data-neta={campo}
              ...
              <input
                value={item.cantidadNeta}
                onChange={(e) => onUpdate(campo, i, "cantidadNeta", formatearEntero(e.target.value))}
                onFocus={(e) => setTimeout(() => e.target.select(), 0)}
                onKeyDown={(e) => {
                  if (e.key === "Enter") {
                    e.preventDefault();
